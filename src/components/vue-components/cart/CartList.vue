<template>
    <section
        ...
        <table v-if="isLoaded && cartList?.length > 0"
            class="table-auto rounded shadow-md bg-base-200 w-screen px-4 md:px-0 md:w-1/2 2xl:w-1/3 mx-auto overflow-hidden">
            <thead>
            ...
            <tbody>
                <tr v-for="(item) in (cartList as CartList[])" :key="item.id_product">
                    ...
                    <td class="py-4 px-4">
                        <button class="text-error hover:text-red-600 btn-ghost rounded mx-auto"
                            @click="deleteItem(item.id)">
                            <svg class="w-6 h-6 text-error dark:text-white" aria-hidden="true"
        ...
        <div class="flex flex-row-reverse justify-between w-screen px-4 md:px-0 md:w-1/2 2xl:w-1/3">
            ...
            <div class="mt-6">
                <button class="btn btn-primary text-white py-2 px-4 rounded"
                    :class="{ 'btn-disabled': cartList?.length === 0 }" :disabled="cartList?.length === 0"
                    @click="checkout">
                    Proceed to Checkout
                </button>
            </div>
        ...
        if (error) {
            ...
            window.snap.pay(data, {
                ...
                onSuccess: async function () {
                    await actions.transactions.createTransaction(cartList.value);
